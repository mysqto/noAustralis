...
classicthemerestorerjs.ctr = {
                    ...
                    switch (name) {
                        ...
                        case "nbiconsize":
                            ...
                            if(currentAttribute=="small" && selectedAttribute=="small"){}
                                ...
                                // needs a delay or Cyberfox will override it on restart
                                setTimeout(function(){
                                    try {
    ...
    updateTabWidth: function() {
        ...
        //if(minWidthValue<32) minWidthValue=32;
        //if(maxWidthValue<32) maxWidthValue=32;
...
function applyNewSheet(sheet){
    ...
    //Appmenu Items Clean Ram | Restart Browser
        ...
        document.getElementById("appmenu-popup")
            .addEventListener("popupshowing", function (e) {
                ...
                try{		
                    ...
                    var restartIsEnabled = false;
                    var ramIsEnable = false;

                    var restartBrowMenu = document.getElementById("appmenu_restartBrowser");
                    var ramItemA = document.getElementById("appmenu_minimizeMemoryUsage");

                    if(prefServiceBranch.getPrefType('browser.restart.enabled')){
                        restartIsEnabled = Services.prefs.getBoolPref("browser.restart.enabled");  

                    }    


                    if(prefServiceBranch.getPrefType('clean.ram.cache')){
                        var ramIsEnabled = Services.prefs.getBoolPref("clean.ram.cache");

                    }
                    //R.M.F	
                    if (restartIsEnabled){
                        restartBrowMenu.removeAttribute('hidden', false);
                    }else{
                        restartBrowMenu.setAttribute('hidden', true);		
                    }	

                    //R.A.M
                    if (ramIsEnabled){
                        ramItemA.removeAttribute('hidden');
                    }else{
                        ramItemA.setAttribute('hidden', true);
                    }	

                }catch (e){
                    //Catch any nasty errors and output to dialogue and console
                    alert("A Error Was Encountered While Trying To Adjust Ram Cache And Restart Browser ID:C6756!" + e);
                }	
